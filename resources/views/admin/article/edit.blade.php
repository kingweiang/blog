...
@section('content')
    ...
    <div class="result_wrap">
        <form action="{{url('admin/article/'.$field->art_id)}}" method="post">
            {{csrf_field()}}
            <input type="hidden" name="_method" value="put"> {{--使用put方法--}}
            <table class="add_tab">
                <tbody>
                    <tr>
                        <th width="120">分类：</th>
                            ...
                            <script type="text/javascript">
                                ...
                                $(function() {
                                    $('#file_upload').uploadify({
                                        'buttonText':'图片上传',
                                        'formData'     : {
                                            'timestamp' : '<?php echo $timestamp;?>',  {{--传入时间戳--}}
                                            '_token'     : "{{csrf_token()}}"
                                        },
                                        ...
                                        'onUploadSuccess':function (file,data,response) {
                                           $('input[name=art_thumb]').val(data);   //  将文件路径写入input框里面
                                            $('#art_thumb_img').attr('src','/'+data);  //  显示缩略图，用'/'+才能调用跟目录的文件
                                            alert('The file'+ file.name + 'was successfully uploaded with a repsonse of'+response+':'+data);
                                        }
                                    });
                                });
                            </script>
